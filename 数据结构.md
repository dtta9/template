...
别名：$Chtholly\;Tree$ 也叫老司机树 $ODT\;(Old\;Driver\;Tree)$.

用处：

​	在保证出现数据推平也就是将一段区间全部赋值为一个数的操作 以及 数据随机的情况下可以使用 珂朵莉树。
...
例题：支持两种操作，一种是合并两个堆，一种是删除第 x 个元素所在堆的堆顶元素。
